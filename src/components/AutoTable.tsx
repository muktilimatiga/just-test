...
interface AutoTanStackTableProps<T> {
  data: T[];
  pageSize?: number;
  // A map where key = column name, value = custom ColumnDef
...
export function AutoTanStackTable<T extends object>({ 
  ...
  const columns = useMemo(() => {
    ...
    const generatedColumns = keys.map((key) => {
      // If an override exists for this key, use it!
      if (columnOverrides[key as keyof T]) {
        return columnOverrides[key as keyof T]!;
      }
      
      // Default Auto-Column Logic
      return {
        header: key.charAt(0).toUpperCase() + key.slice(1).replace(/_/g, ' '),
        accessorKey: key,
        cell: (info: any) => {
          const val = info.getValue();
          // specific handling for dates if detected, otherwise stringify
